	...
	private BinaryTreeNode<Integer>  removeHelper(BinaryTreeNode<Integer> node, int input) {
		...
		if(node.data==input) {
			if(node.left==null && node.right ==null) {
				 ...
				 * it is possible to have two or more same maximum data on left
				 * & left can't be equal to root data so we chose minimum from right  
				 */
				node.data = rightMin;
...
//			return node;
	...
	private int findMinimum(BinaryTreeNode<Integer> node) {
		if(node == null) {
