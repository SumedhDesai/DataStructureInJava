 ...
 * Maximum Profit on App
	...
	Price of your app should be Rs. 20 or Rs. 30. For both prices, you can get the 
	profit Rs. 60.
	Sample Input 2 :
	5
	34 78 90 15 67
...
public class MaximumProfitOnApp {
	...
	private static int maximumProfit(int[] budget) {
		int maxProfit=Integer.MIN_VALUE;
		Arrays.sort(budget);
		for(int i=0; i<budget.length; i++) {
//			Giving weightage to each price in a such a way that
//			low price have high weightage.
			budget[i]=budget[i]*(budget.length-i);
		}
		for(int i=0; i<budget.length; i++) {
			if(maxProfit<budget[i]) {
				maxProfit=budget[i];
			}
		}
		return maxProfit;
