 ...
 * Longest Consecutive Sequence
	...
	longest consecutive sequence. If the length of longest consecutive sequence, 
	...
	Sample Output 2 :
	7 9
	Explanation: Sequence should be of consecutive numbers. Here we have 2 sequences with 
	same length i.e. [1, 2, 3] and [7, 8, 9], but we should select [7, 8, 9] because the 
	starting point of [7, 8, 9] comes first in input array and therefore, the output will 
...
//					&& !visited.get(temp)
				...
				visited.put(num, true);
				count++;
				startIndexAns = index.get(temp);
				temp=temp-1;
			}

			if(count > globalMaxLength) {
				globalMaxLength=count;
				globalStartIndexAns=startIndexAns;
			}else if(count == globalMaxLength) {
				if(globalStartIndexAns>startIndexAns) {
					globalStartIndexAns=startIndexAns;
				}
			}
		}
		int startNum=arr[globalStartIndexAns];
		ans.add(startNum);
		if(globalMaxLength>1) {
			ans.add(startNum+globalMaxLength-1);
		}
		return ans;
	}
